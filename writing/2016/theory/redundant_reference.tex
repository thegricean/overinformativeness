...
\caption{Example contexts in which different levels of reference are necessary for establishing unique reference to the target marked with a green border: (a) sub (\emph{dalmatian}), (b, c) basic (\emph{dog}), or (d) super (\emph{animal}).\ek{I would make the item descriptions clearer (instead of item12,...) or leave them out}}
...
The literal listener $L_0$ observes an utterance $u$ from the set of  utterances $U$, consisting of single adjectives denoting features available in the context of a set of objects  $O$, and returns a distribution over objects $o \in O$. Here \ek{sometimes you put commas after "here", sometimes not -- not sure whether that's on purpose (same holds for comma in front of "which")}, $\mathcal{L}(u,o)$ is the lexicon that encodes deterministic lexical meanings such that: 
