...
\caption{For each target, proportion of color-only (\emph{yellow}), type-only (\emph{banana}), color-and-type (\emph{yellow banana}), and other (\emph{funky carrot}) utterances as a function of mean object typicality for the type-only utterance (e.g., \emph{banana}), across conditions. \emph{\textsc{color} banana} cases are circled in their respective color.\jd{elisa, please update this plot so that the utterance labels correspond to exactly what's in the text (ie: color-and-type, type-only, color-only, other), the x-axis reads "Typicality of object for type-only utterance", and the panels are less flat}}
