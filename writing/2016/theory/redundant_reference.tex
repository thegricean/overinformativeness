...
\jd{pragmatic speaker with non-deterministic semantics for noun choice}

In order to model reference level selection, we extend RSA by simply relaxing the assumption of a deterministic semantics. As in the basic RSA model \cite{frank2012, goodmanstuhlmueller2013}, the speaker seeks to be informative with respect to an internal model of a literal listener. This listener updates her beliefs to rule out possible worlds that are inconsistent with the meaning of the speaker's utterance. This is the innovation: rather than assuming that words have deterministic truth conditions, we assign each word a graded meaning. For instance, the word ``dog'' describes a dalmatian better than a grizzly bear, but it also describes a grizzly bear better than a tennis ball.
...
To test the model's predictions, we conducted an interactive web-based written production study within a reference game setting.\footnote{See \appref{app:replication}  for a validation of the general paradigm, in which we qualitatively replicate the findings of \citeA{gatt2011} with a different set of stimuli.} \jd{already introduce general paradigm earlier in the noun choice part?} Speakers and listeners were shown arrays of objects of that could vary in color and size. Speakers were asked to produce a referring expression to allow the listener to identify a target object. We manipulated the number of distractor objects in the grid, as well as the variation in color and size among distractor objects.
...
\red{Fillers were target trials from a different experiment \cite{graf-underreview}. Each filler item contained a three-object grid. None of the filler objects occurred on target trials. Objects stood in various taxonomic relations to each other and required neither size nor color mention for unique reference. For example, a filler trial could consist of a display with a dalmatian, a pug, and a squirrel.} \jd{update this, actually write up as its own section}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{../../../models/1_basic_overinformativeness/results_bda/graphs/scenevariation-fixed-reducedconditions-fullutts}
\caption{Model posterior predictive XXX estimate of utterance probability (blue) alongside empirical utterance proportions  (red) as a function of scene variation. Rows indicate the sufficient dimension.}
\label{fig:modelexpresults}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=.8\textwidth]{../../../models/1_basic_overinformativeness/results_bda/graphs/predictives-collapsed-fixed-reducedconditions-fullutts}
