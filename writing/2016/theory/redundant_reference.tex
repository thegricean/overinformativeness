...
\begin{table}
%\centering
\caption{Row-wise literal listener distributions $P_{L_0}(o | u)$ for each utterance $u$ in the context depicted in \figref{fig:sizesufficient}, allowing only one-word utterances (left) or one- and two-word utterances (middle) under a deterministic semantics, or one- and two-word utterances under a non-deterministic semantics (right).\jd{fill in non-deterministic values}}
...
& $o_{\textrm{big\_blue}}$ & $o_{\textrm{big\_red}}$ & $o_{\textrm{small\_blue}}$ \\
\midrule
\emph{big} & .5 & .5 & 0 & .5 & .5 & 0 & & & \\
\emph{small} & 0 & 0 & 1 & 0 & 0 & 1 & & & \\
\emph{blue} & .5 & 0 & .5 & .5 & 0 & .5 & & & \\
\emph{red} & 0 & 1 & 0 & 0 & 1 & 0 & & & \\
\emph{big blue} & NA & NA & NA & 1 & 0 & 0 & & & \\
\emph{big red} & NA & NA & NA & 0 & 1 & 0 & & & \\
\emph{small blue} & NA & NA & NA & 0 & 0 & 1 & & & \\
\bottomrule
\end{tabular}
\label{tab:detliteral}
\end{table}

The pragmatic speaker in turn produces an utterance proportional to the utility of that utterance:
...
\includegraphics[width=\textwidth]{pics/modelexploration-fidelityeffect-paper}

\caption{Probability of producing sufficient \emph{small pin}, insufficient \emph{blue pin}, and redundant \emph{small blue pin} contexts as depicted in \figref{fig:sizesufficient}, as a function of semantic value of color and size utterances (for $\alpha = 30$ and $ \beta_c = 0$).}
