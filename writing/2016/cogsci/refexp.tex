...
Finally, classic work on concepts has shown that objects within a category vary in \emph{typicality}, and that there exists a privileged level of abstraction called the basic-level toward which speakers tend to gravitate in free-response naming tasks \cite{RoschEtAl76_BasicLevel}. 
...
\todo[inline]{rdh: I think we need to say something about how we're going to try to account for basic-level biases through these other factors, rather than building it in -- if it were up to me, I'd probably frame the whole paper more as ``how can we push around preferences for the basic level by manipulating context and cost?''}
...
%\paragraph{\bf Design}
Each trial consisted of a display of three images, one of which was designated as the target object. Every pair of participants saw every target exactly once, for a total of 36 trials per pair. The distractor items were selected from four different context conditions, corresponding to different communicative pressures (see Fig. \ref{fig:design}). We refer to these conditions with pairs of numerals specifying which levels of the taxonomy are present in the distractors: (a) \textbf{item12}: one distractor of the same basic level and one distractor of the same superlevel (e.g. target: ``dalmatian'', distractor 1: ``greyhound'', distractor 2: ``squirrel''), (b) \textbf{item22}: two distractors of the same superlevel, (c) \textbf{item23}: one distractor of the same superlevel and one unrelated item and (d) \textbf{item33}: two unrelated items. \todo[inline]{rdh: would be nice to use the TeX \emph{description} environment to format this if we have room} Each pair saw nine trials in each condition. \todo[inline]{rdh: maybe it should already be clear from this, but how did we assign contexts to the 36 items? Totally randomly (with this 9 trials/condition constraint)? Or did the four targets within a domain get a one-to-one mapping with the four conditions?} Furthermore, the experiment contained 36 filler items, in which participants were asked to produce referential expressions for objects which differed only in size and color. Images from filler trials were not reused on target trials. Trial order was randomized. 
...
\todo[inline]{rdh: i don't think these numbers  add up to 100... 1.2 were incorrect and excluded, (66.2 + 13.8) were correct and included, 10\% of the total correct ones were excluded $= 91.2$ at most. Or maybe I'm counting wrong, in which case we should maybe find a less confusing way of expressing these probabilities?}

\paragraph{Typicality norms}

To estimate the influence of typicality on speaker behavior, we recruited 240 participants through Mechanical Turk. On each trial, we presented participants with an image from the main experiment and asked them ``How typical is this for X?'', where X was a category label at the sub-, basic-, or super- level. They then adjusted a slider bar ranging from \emph{not at all typical} (coded as value 0) to \emph{very typical} (coded as 1). 
...
Each participant provided typicality ratings for 7 \emph{target}, 10 \emph{distdiffsuper}, and 28 \emph{distsamesuper} cases (randomly sampled from the total set of items). Each case received between 6 and 27 ratings.  Average untransformed slider values between 0 and 1 were used as the typicality values throughout our results. 
...
We relate this to our empirically elicited typicality norms via an exponential relationship: $\denote{l}(o)=\exp(\text{typicality}(o,l))$.
...
To determine the relative importance of various factors within our speaker model, we conducted a Bayesian data analysis. We implemented the model in webppl, a probabilistic programming language \cite{GoodmanStuhlmuller14_DIPPL}, and inferred posteriors for the three free parameters using Markov Chain Monte Carlo (MCMC), conditioning on the production data from the experiment reported above. More specifically, for every iteration of the chain, we generated the probability of a speaker using each level of reference (i.e .``sub'', ``basic'', or ``super'') for every \emph{context} that participants could encounter in the task, and then computed the likelihood of the actual expressions that participants used. We assumed uniform priors for each parameter:
$$\begin{array}{lcl}
\alpha & \sim & Unif(0,20) \\
\beta_f & \sim & Unif(0,5) \\
\beta_l & \sim & Unif(0,5)
...
\caption{Posterior distribution over model parameters.% $\lambda$, $\beta_f$, $\beta_l$
}
\label{fig:paramposteriors}
\end{figure}

Posteriors for these parameters are presented in Fig. \ref{fig:paramposteriors} and posterior predictives, compared to empirical estimates, are presented in Fig. \ref{fig:scatterplot}. On the most fine-grained, by-target, level the model achieves a correlation of $r = .79$ by rating informativeness relatively strongly (MAP $\lambda = 10.8$, HPD $= [9.7, 12.8]$) and weighting length overall somewhat more strongly than frequency (MAP $\beta_l = 2.5$, HPD $= [1.9, 3.1]$, MAP $\beta_f = 1.3$, HPD $= [0.8, 1.8]$). Correlating model predictions and empirical proportions on the by-domain level (collapsing across targets) and on the by-condition level (further collapsing across domains, as in \figref{fig:qualitativemodel}) yields correlations of .88 and .96, respectively. That is, when collapsing across items the model explains most of the variance in our data; this suggests that remaining deviations are due to unaccounted for domain or target effects (see Discussion).
...
In order to ascertain whether typicality was indeed contributing to the explanatory power of the model, we ran a version with an additional typicality weight parameter $\beta_t \in [0,1]$. This parameter interpolated between empirical typicality values and an \emph{a priori} truth-conditional function returning one if the object was a member of the label category and zero otherwise. The posterior distribution of this parameter provides a simple model comparison: if $\beta_t = 0$ is excluded from the HPD, and the distribution skews high, then some influence of typicality is necessary to account for the data. We found a MAP estimate of .94, HPD $= [0.88,1]$, strongly indicating that empirical typicality values are necessary.  Finally, we ran a model including a parameter weighting the \emph{product} of frequency and cost, corresponding to the interaction term in our regression analysis, but its posterior distribution was strongly peaked at 0, indicating that any contribution of the interaction is captured by other aspects of the model. 
...
\caption{Empirical proportions against model posterior predictive.}
...
There was, however, some item-wise variation that the model left unexplained. For instance, candy items like ``gummy bears'' or ``jelly beans'' were referred to by their sub level term exceptionally often.
...
Importantly, the model predicts a well-documented preference for speakers  to refer to objects at the basic level when not constrained by contextual considerations  \cite{RoschEtAl76_BasicLevel}. In our model, this preference emerges naturally from cost considerations: basic-level labels tend to be shorter and more frequent than sub and super level terms. However, speakers did not always use the basic level term, even when unconstrained. In certain cases where object typicality was relatively high for the sub level term compared to the basic level term, that term was preferred (as was the case for e.g. ``panda bear''), suggesting an interesting interplay between typicality and level of abstraction. % \jd{give good qualitative example in results section that you can pick up again here?}
...
\todo[inline]{rdh: this paragraph kind of bothers me: a theory evoking underlying conceptual structure seems more parsimonious to me, and our results about cost in language seem kind of circular at that level of explanation... Tried to put in this last sentence to feel less conflicted about it, but maybe that just makes it worse}
...
\todo[inline]{rdh: not a great note to end on...}
%
%\begin{itemize}
