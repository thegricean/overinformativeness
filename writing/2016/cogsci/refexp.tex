...
Finally, classic work on the psychology of concepts suggests that \emph{typicality} of a referent within it's category effects the choice of reference \cite{RoschEtAl76_BasicLevel}. In particular, people will generally choose to refer at the basic level, but may become more specific for atypical items.
...
To evaluate the impact of these three factors---contextual informativity, utterance cost, and typicality---we constructed a two-player online game. Participants saw a common context of objects, one was indicated as the referent only to the speaker, who was asked to communicate this object to the listener. The listener then chose among the objects. Critically, the speaker and listener communicated by free use of a chat window, allowing us to gather relatively natural referring expressions. We manipulated the category of distractor objects and used items that vary in label complexity and typicality. This allows us to evaluate whether each factor influences the referring expressions generated by participants; it doesn't predict their quantitative interactions.
...
%
The RSA framework has been applied to many language interpretation tasks \cite{stuff}, but relatively rarely to production data.
...
Speakers' and listener's messages were first parsed automatically, that is the referential expression used by the speaker was extracted for each trial and it was checked whether the utterance contained the current target's correct sub, basic or super level term. Thereby 66.2\% of correct trials were labelled as mentioning a level of reference. In the next step, utterances were checked manually to determine whether they contained a correct level of reference term, which was not detected by the parsing algorithm due to typos or grammatical modification of the expression. In this way, meaning-equivalent alternatives such as ``doggie'' for ``dog'',  or contractions such as ``gummi'',``gummies'' and ``bears'' for ``gummy bears'' were counted as containing a level of reference term. By this means another 13.8\% of trials were included in the analysis. A total of 20.0\% of correct trials were excluded, because the utterance consisted of an attribute of the superclass (``the living thing'' for ``animal''), of the basic level (``can fly'' for ``bird''), of the subcategory (``barks'' for ``dog'') or of the  particular instance (``the thing facing left'') was mentioned rather than a level of reference term. These kinds of attributes were also sometimes mentioned additionally to the level of reference term in the 80.0\% of correct trials which were included in the analysis, that is 4.0\% of mentioned sub level terms, 12.6\% of basic level terms and 46.2\% of super level terms contained an additional modifier. By making use of attributes,  speakers could generate unambiguous referential expression despite using a level of reference which would by themselves be insufficient for disambiguation, e.g. by referring to a dalmatian as a ``spotted dog'' (in the context of another dog being present, for instance a pug which is not spotted). Furthermore, 0,5\% of correct trials mentioned two different levels of reference, in this case both levels of reference were counted as being mentioned in this trial. Additional processing also established the number of correct trials where a determiner (``the'' or ``a''/``an'') or an indefinite referent such as ``one'', ``thing'' or ``object'' was used, how many correct trials consisted of complete sentences and the number of correct trials where level of reference terms were contracted (namely 2.7\%, 0.9\%, 1.0\% and 5.3\% respectively).
...
As in earlier Rational Speech Act (RSA) models \cite{frank2012, goodmanstuhlmueller2013}, the speaker seeks to be informative with respect to an internal model of a literal listener. This listener updates her beliefs to rule out possible worlds that are inconsistent with the meaning of the speaker's utterance. Rather than assuming that labels have deterministic truth-conditions, as has usually been done in the past, we account for typicality by allowing each label a graded meaning. For instance, the label ``dog'' fits a dalmatian better than a grizzly bear, but it also fits a grizzly bear better than a tennis ball.
The speaker also seeks to be parsimonious: the speaker utility includes both informativiity and cost of the label, which we allow to include both length and frequency.

Formally, we start by specifying a literal listener $L_0$ who hears a label $l$ in the context of some set of objects $\mathcal{O}$ and forms a distribution over the referenced object, $o \in \mathcal{O}$ : 
$$P_{L_0}(o | l) \propto \denote{l}(o).$$
Here $\denote{l}(o)$ is the lexical meaning of the label $l$ when applied to object $o$. We take this to be a real number indicating the degree of acceptability or typicality of object $o$ for category $l$, for which we use the empirical typicality norms elicited earlier. \ndg{fix up wrt norms refernce}

Next, we specify a speaker $S_1$ who is attempting to refer to a particular object $o \in \mathcal{O}$ and chooses among possible labels $l \in \ell(o)$: 
$$P_{S_1}(l | o) \propto \exp\{\alpha \ln P_{L_0}(o | l) + \beta_1 \hat{c}_f  + \beta_2 \hat{c}_l \}$$
...
Length cost $c_l$ is defined as the number of characters in the label, and frequency cost $c_f$ is estimated using the log probability of occurring in the Google Books corpus from 1960 to the present. \ndg{are the length and freq defined in the previous section? interaction term?}
The relative importance of these three factors is determined by three parameters that we will fit to the data.
