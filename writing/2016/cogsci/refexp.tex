...
We recruited 56 participants over Amazonâ€™s Mechanical Turk, who were all native speakers of English and who were reimbursed for their participation in the experiment. The subjects participated in pairs of two.
...
\section{\bf Modeling basic-level reference}

To show that we can account for these context effects purely through communicative pressures, we formulated a simple probabilistic model of basic-level reference. As in earlier Rational Speech Act (RSA) models \cite{frank2012, goodmanstuhlmueller2013}, the speaker seeks to be maximally informative with respect to an internal model of a literal listener. That listener updates their beliefs to rule out possible worlds that are inconsistent with the meaning of the speaker's utterance. We incorporate length and frequency as costs that affect the speaker's likelihood of using a label, and we account for typicality as part of a label's graded meaning: the label ``dog'' fits a dalmatian better than a grizzly bear, but it also fits a grizzly bear better than a tennis ball.
...
$$P_{L_0}(o_i | l) \propto \exp\{\alpha P(o_i)  U_{lex}(l; o_i)\}$$
where $P(o_i)$ is taken to be a uniform prior over objects, $U_{lex}(l; o_i)$ is the lexical meaning of the label $l$ when applied to object $o_i$, and $\alpha$ is an optimality parameter\footnote{$\alpha = 1$ corresponds to a soft-max decision rule, and $\alpha \rightarrow \infty$ corresponds to a optimal hard-max decision rule.}.

Next, we specify a speaker $S_1$ who is attempting to refer to a particular object $o \in \mathcal{O}$ and returns a response distribution over possible labels $l_j \in \ell(o)$: 
$$P_{S_1}(l_j | o) \propto \exp\{\alpha P_{L_0}(o | l_j)P(l_j) U_{cost}(l_j)\}$$
where $P(l_j)$ is the prior likelihood of using label $l_j$, $U_{cost}(l_j)$ is the label cost function and the other terms are as defined above. 
\todo[inline]{rdh: this exposition would be nicer if we incorporated cost into the label prior}

Finally, we must define the the lexical meaning function $U_{lex}(l; o_i)$, the cost function $U_{cost}(l_j)$, and the label prior $P(l_j)$. 

\todo[inline]{rdh: we'll probably want to write down how we compute length and frequency earlier, since some results depend on them.}
