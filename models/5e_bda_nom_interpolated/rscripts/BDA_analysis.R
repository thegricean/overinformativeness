setwd("/Users/titlis/cogsci/projects/stanford/projects/overinformativeness/models/5e_bda_nom_interpolated/")
library(ggplot2)
library(dplyr)
library(coda)
library(purrr)
...
modelversion = "fulldataset-typicalities-interpolated-hmc"
...
ggplot(numericSubset, aes(x=value)) +
  ...
  geom_histogram(data=subset(numericSubset, parameter == "beta_t"),
                 binwidth = (range(numericSubset[numericSubset$parameter == "beta_t",]$value)[2] - range(numericSubset[numericSubset$parameter == "beta_t",]$value)[1])/bw, colour="black", fill="white")+  
...
ggsave(paste("/Users/titlis/cogsci/projects/stanford/projects/overinformativeness/models/5e_bda_nom_interpolated/graphs/parameterposteriors-",modelversion,".pdf",sep=""),height=1.5,width=5)
...
predictive.samples$ModelType = "interpolated"

write.table(predictive.samples,paste("predictive-barplot-",modelversion,".txt"),row.names=F,quote=F)
# change name for plotting
predictive.samples$ModelType = "model"

# get correlation
gof(agr_noattr$Probability,predictive.samples$Probability) #r=.94,r2=.89
...
ggsave(paste("/Users/titlis/cogsci/projects/stanford/projects/overinformativeness/models/5e_bda_nom_interpolated/graphs/qualitativepattern-",modelversion,".pdf",sep=""),height=3.5,width=5)
...
# get correlation
gof(predictive.samples$EmpProportion,predictive.samples$MAP) #r=.83,r2=.69
...
ggsave(paste("/Users/titlis/cogsci/projects/stanford/projects/overinformativeness/models/5e_bda_nom_interpolated/graphs/scatterplot-",modelversion,".pdf",sep=""),height=4,width=6)
