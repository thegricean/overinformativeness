...
setwd("~/Repos/overinformativeness/models/basic_level_reference/")
...
d = read.csv("normalizedFreqLength.csv",quote="")
str(d)
nrow(d)
head(d)
summary(d)
...
head(cors_noattr)
cors_noattr[cors_noattr$Cor == max(cors_noattr$Cor),] # maximized correlation for alpha = .8 and lengthWeight = .15 and freqWeight = .5 (.73)

ggplot(cors_noattr, aes(x=alpha,y=Cor,color=as.factor(lengthWeight))) +
  geom_point() +
  facet_wrap(~freqWeight) +
  ggtitle("Max r=.63 for alpha=4, lengthWeight = .5, freqWeight=2.5")
ggsave("graphs/reparameterized/correlations_noattr_multiplicative.pdf",height=8,width=10.5)
...
cors_noattr_bydomain %>%
  group_by(domain) %>%
  summarise(bestcorr=max(Cor),bestalpha=alpha[Cor==max(Cor)],bestlengthWeight=lengthWeight[Cor==max(Cor)],bestfreqWeight=freqWeight[Cor==max(Cor)])
...
cors_allattr[cors_allattr$Cor == max(cors_allattr$Cor),] # maximized correlation for alpha = .8 and lengthWeight = .15 and freqWeigth=.5 (.75)

ggplot(cors_allattr, aes(x=alpha,y=Cor,color=as.factor(lengthWeight))) +
  geom_point() +
  facet_wrap(~freqWeight) +
  ggtitle("Max r=.63 for alpha=4, lengthWeight=.5, freqWeight=2.5")
ggsave("graphs/reparameterized/correlations_allattr_multiplicative.pdf",height=8,width=10.5)
...
cors_noattr_coll[cors_noattr_coll$Cor == max(cors_noattr_coll$Cor),] # maximized correlation for alpha = .75 and lengthWeight = .16 and freqWeight=0 (.85)

ggplot(cors_noattr_coll, aes(x=alpha,y=Cor,color=as.factor(lengthWeight))) +
  geom_point() +
  facet_wrap(~freqWeight) +
  ggtitle("Max r=.74 for alpha=.75, lengthWeight=.16, freqW=.5 (collapsed)")
ggsave("graphs/reparameterized/correlations_noattr_collapsed_multiplicative.pdf",height=7,width=10.5)
...
cors_allattr_coll[cors_allattr_coll$Cor == max(cors_allattr_coll$Cor),] # maximized correlation for alpha = .85 and lengthWeight = .14 and freqWeight=.5 (.86)

ggplot(cors_allattr_coll, aes(x=alpha,y=Cor,color=as.factor(lengthWeight))) +
  geom_point() +
  ggtitle("Max r=.73 for alpha=2.5, lengthWeight=0.5, freqW=2 (collapsed)") +
  facet_wrap(~freqWeight)
ggsave("graphs/reparameterized/correlations_allattr_collapsed_multiplicative.pdf",height=8,width=10.5)

# plot model predictions vs empirical scatterplot for best fitting params
ggplot(d[d$alpha==2.5 & d$lengthWeight==0.5 & d$freqWeight==2,],aes(x=modelProb,y=EmpiricalProbAllAttr,shape=condition,color=Utterance)) +
  geom_point() +
  xlim(c(0,1)) +
  ylim(c(0,1)) +
...
# plot model predictions vs empirical scatterplot for best fitting params
ggplot(d[d$alpha==2.5 & d$lengthWeight==0.5 & d$freqWeight==2,],aes(x=modelProb,y=EmpiricalProbNoAttr,shape=condition,color=Utterance)) +
  geom_point() +
  xlim(c(0,1)) +
  ylim(c(0,1)) +
  ylab("Empirical proportion") +
...
coll = d[d$alpha==2.5 & d$lengthWeight==0.5 & d$freqWeight==2,] %>%
#coll = d[d$alpha==1 & d$lengthWeight==.15 & d$freqWeight==.3,] %>%
...
ggplot(coll,aes(x=modelProb,y=EmpiricalProbAllAttr,shape=condition,color=Utterance)) +
  ...
  geom_abline(xintercept=0,yintercept=0,slope=1,color="gray60") +
  facet_wrap(~domain)
ggsave("graphs/reparameterized/model_empirical_allattr_bydomain_collapsed_multiplicative.pdf",width=8.4,height=6)
...
ggplot(coll,aes(x=modelProb,y=EmpiricalProbNoAttr,shape=condition,color=Utterance)) +
  ...
  geom_abline(xintercept=0,yintercept=0,slope=1,color="gray60") +
  facet_wrap(~domain)
ggsave("graphs/reparameterized/model_empirical_noattr_bydomain_collapsed_multiplicative.pdf",width=8.4,height=6)

best_d_all = d[d$alpha == 2.5 & d$lengthWeight == 0.5 & d$freqWeight==2,] %>%
...
ggsave("graphs/reparameterized/probs_best_all_multiplicative.pdf",width=10,height=3.5)

best_d_no = d[d$alpha == 2.5 & d$lengthWeight == 0.5 & d$freqWeight == 2,] %>%
  group_by(condition,Utterance) %>%
  summarise(Probability=mean(modelProb),ci.low=ci.low(modelProb),ci.high=ci.high(modelProb))
...
p = ggplot(best_d_no, aes(x=condition,y=Probability)) +
  ...
  geom_errorbar(aes(ymin=YMin,ymax=YMax),position=dodge,width=.25)+
  facet_wrap(~Utterance)
ggsave("graphs/reparameterized/probs_best_no_multiplicative.pdf",width=10,height=3.5)


# agr = d[d$alpha %in% c(.01,.05,.1,.31,.91) & d$lengthWeight %in% c(.01,.02,.1,.2,.32),] %>%
#   group_by(condition,alpha,lengthWeight,Utterance) %>%
#   summarise(Probability=mean(modelProb),ci.low=ci.low(modelProb),ci.high=ci.high(modelProb))
# agr = as.data.frame(agr)
# agr$YMin = agr$Probability - agr$ci.low
# agr$YMax = agr$Probability + agr$ci.high
...
#   geom_bar(stat="identity",position=dodge) +
#   geom_errorbar(aes(ymin=YMin,ymax=YMax),position=dodge,width=.25) +
#   facet_grid(alpha~lengthWeight)
# ggsave("graphs/reparameterized/probs.pdf",width=10,height=10)
