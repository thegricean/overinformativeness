...
client_onjoingame = function(num_players, role) {
  ...
  if(role === "speaker") {
    ...
    $('#instructs').append("Click on the target object which is described by the speaker. Please do not refresh page!")
    $("#submitbutton").remove();
  }
  ...
  if(num_players == 1)
    game.get_player(my_id).message = 'Waiting for other player to connect... Please do not refresh this page now or at any other point in the game!'; //why does this need to be on?
...
function mouseClickListener(evt) {
  ...
  for (i=0; i < game.objects.length; i++) {
    ...
    if (hitTest(obj, mouseX, mouseY)) {
      game.socket.send("clickedObj." + obj.name);
    }
  }
};
