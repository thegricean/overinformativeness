...
# exclude greebles (fillers) 
production = droplevels(production[production$condition == "overinformative",])
prop.table(table(production$ColorMentioned,production$ItemMentioned))
production[!production$ColorMentioned & !production$ItemMentioned,c("response","condition","workerid")]
...
m = glmer(ColorMentioned ~ binaryTypicality + (1|workerid) + (1|target_item), data=production, family="binomial")
summary(m)
