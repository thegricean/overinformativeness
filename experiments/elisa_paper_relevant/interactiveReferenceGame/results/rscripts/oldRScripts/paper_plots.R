...
production$OtherComplete = ifelse(production$ColorAndCat | production$Cat | production$Neg | production$Description | production$ColorModifier | production$Other, 1, 0)
...
# plot utterance choice proportions by typicality
agr = production %>%
  select(Color,Type,ColorAndType,OtherComplete,NormedTypicality,context) %>%
...
# change order of Utterance column
agr$Utterance <- as.character(agr$Utterance)
agr$Utterance <- factor(agr$Utterance, levels=c("Type", "Color", "ColorAndType", "OtherComplete"))
...
ggplot(agr, aes(x=NormedTypicality,y=Probability,color=Utterance)) +
  ...
  facet_wrap(~context) +
  scale_color_discrete(name="Utterance",
                       breaks=c("Type", "Color", "ColorAndType", "OtherComplete"),
                       labels=c("Only Type", "Only Color", "Color + Type", "Other")) +
  theme(axis.title=element_text(size=14,colour="#757575")) +
