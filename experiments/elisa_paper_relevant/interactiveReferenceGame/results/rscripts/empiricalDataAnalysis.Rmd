---
title: "Typicality Effects in Reference Game"
output: html_notebook
---
## Introduction

Some introductory words about the study
...
## Import, clean and categorize empirical data
...
#### Spelling correction and categorization of referential expression production
...
#### Histogram of mentioned features by context
...
#### Plot of utterance choice proportions with error bars
...
#### Plot utterances by typicality but dependent on distractor typicality
Prepare data frame for visualization
Values are added to the data frame that describe the similarity of one distractor to the target object. This values were taken from the norming study asking for "How typical is this object for a COLOR TYPE?". If at least one of the distractors has a typicality value >= 0.1, the context is classified as a simDist-context (i.e., context with a similar distractor). Informative contexts are excluded because they are all considered simDist-contexts due to the high similarity of same-type distractors.

```{r}
# add correct distractor names
dists = read.csv(here("data","dist_lexicon.csv"))
...
comboTyp = read.csv(here("..","..","norming_full","results","data","meantypicalities.csv"))
comboTyp = droplevels(comboTyp[,c("Utterance","Combo","RoundMTypicality")])

distBins = production[,c("context","Color","Type","ColorAndType","Other","target","Dist1_rev","Dist2_rev","NormedTypicality")]
distBins = merge(distBins,comboTyp,by.x=c("target","Dist1_rev"),by.y=c("Utterance","Combo"))
distBins = merge(distBins,comboTyp,by.x=c("target","Dist2_rev"),by.y=c("Utterance","Combo"))
distBins$TargetTypicality = distBins$NormedTypicality
distBins$Dist1Typicality = distBins$RoundMTypicality.x
distBins$Dist2Typicality = distBins$RoundMTypicality.y
...
Plot results from contexts with at least one distractor which is similar to the target
...
Plot results from contexts in which both distractors show little similarity to the target
