...
ctyp = read.table(file="../../23_color_patch_norming/results/data/meantypicalities.csv",sep=",",header=T)
ctyp = ctyp[as.character(ctyp$Item) == as.character(ctyp$utterance),]
row.names(ctyp) = paste(ctyp$Color,ctyp$Item)
head(typ)
nrow(typ)
...
m = glmer(ColorMentioned ~ cNormedTypicality*cInformative*cCC + (1+cNormedTypicality+cInformative|gameid) + (1|Item), data = centered, family="binomial")
summary(m)
